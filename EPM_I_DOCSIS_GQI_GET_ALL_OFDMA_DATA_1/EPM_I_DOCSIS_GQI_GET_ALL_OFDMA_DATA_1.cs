...
public class CmData : IGQIDataSource, IGQIInputArguments, IGQIOnInit
{
        ...
        try
        {
            ...
            switch (ofdmaInformation)
            {
                case "OFDMA Channels":
                    ...
                    Dictionary<string, CcapOfdmaOverview> ccapRows = ExtractCcapChannelsData(ccapTable, backEndHelper.CcapId);
    ...
    private static Dictionary<string, CcapOfdmaOverview> ExtractCcapChannelsData(List<HelperPartialSettings[]> ccapTable, string ccapId)
    {
            ...
            List<GQICell> listGqiCells = new List<GQICell>
                {
                    ...
                    new GQICell
                    {
                        Value = ParseStringValue(ccapRow.Value.CcapId),
                    },
                };

            var dmaIdParts = ccapRow.Value.CcapId.Split('/');

            var paramIdMetadata = new ObjectRefMetadata { Object = new ParamID(Convert.ToInt32(dmaIdParts[0]), Convert.ToInt32(dmaIdParts[1]), 5505, ccapRow.Value.OfdmaId) };

            var rowMetadata = new GenIfRowMetadata(new[] { paramIdMetadata });
